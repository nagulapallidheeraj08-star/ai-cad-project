...
# Install sentencepiece for CodeT5 tokenizer BEFORE importing transformers
subprocess.check_call([sys.executable, "-m", "pip", "install", "-q", "sentencepiece", "tokenizers"])
    ...
    if os.path.exists(OUTPUT_DIR):
        ...
        if checkpoints:
            ...
            if os.path.exists(os.path.join(checkpoint_path, "trainer_state.json")):
                print(f"✓ trainer_state.json found - will resume properly")
            else:
                print(f"⚠ WARNING: No trainer_state.json in {checkpoint_path}")
        else:
